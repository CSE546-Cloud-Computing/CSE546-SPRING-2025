...
class iam_policies():
    ...
    def validate_lambda(self, attached_policies):
        ...
        if "AWSLambda_ReadOnlyAccess" in attached_policies:
            policy_exits = True
            self.print_and_log(self.logger, "[IAM-log] AWSLambda_ReadOnlyAccess policy attached with grading IAM")
        else:
            policy_exits = False
            self.print_and_log(self.logger, "[IAM-log] AWSLambda_ReadOnlyAccess policy NOT attached with grading IAM")
    ...
    def validate_iot_greengrass(self, attached_policies):

        if "AWSGreengrassReadOnlyAccess" in attached_policies:
            policy_exits = True
            self.print_and_log(self.logger, "[IAM-log] AWSGreengrassReadOnlyAccess policy attached with grading IAM")
        else:
            policy_exits = False
            comments = "[IAM-log] AWSGreengrassReadOnlyAccess policy NOT attached with grading IAM"
            self.print_and_log_error(self.logger, comments)
    ...
    def validate_policies(self):
        ...
        try:
            ...
            lambda_ro_access_flag        = self.validate_lambda(policy_names)
            iot_greengrass_ro_access_flg = self.validate_iot_greengrass(policy_names)
            return ec2_ro_access_flag, iam_ro_access_flag, sqs_full_access_flag, lambda_ro_access_flag, iot_greengrass_ro_access_flg

        except ClientError as e:
            iam_ro_access_flag           = False
            ec2_ro_access_flag           = None
            sqs_full_access_flag         = None
            lambda_ro_access_flag        = None
            iot_greengrass_ro_access_flg = None
            self.print_and_log_error(self.logger, f"Failed to fetch the attached polices. {e}")
            return iam_ro_access_flag, ec2_ro_access_flag, sqs_full_access_flag, lambda_ro_access_flag, iot_greengrass_ro_access_flg
